...
async function fetchFromAlphaVantage(ticker) {
    ...
    if (!data['Time Series (Daily)']) {
        throw new Error(data['Note'] || data['Information'] || 'Alpha Vantage limit reached or invalid ticker.');
    }
...
router.get('/history/:ticker', async (req, res) => {
    ...
    try {
        const docRef = firebase_admin_1.adminDb.collection('stock_cache').doc(ticker);
...
router.get('/:ticker', async (req, res) => {
    ...
    try {
        ...
        try {
            await firebase_admin_1.adminDb.collection('stock_cache').doc(ticker).set({
                ticker,
